  ...
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
        <svg
          key={star}
          className={`w-3 h-3 ${star <= Math.floor(rating) ? 'text-[#C9A96E]' : 'text-[rgba(201,169,110,0.3)]'}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/4 left-0 w-96 h-96 rounded-full bg-[#C9A96E] opacity-[0.03] blur-[80px]" />
        <div className="absolute bottom-1/4 right-0 w-96 h-96 rounded-full bg-[#1E90FF] opacity-[0.04] blur-[80px]" />
      </div>
      ...
      <div className="relative max-w-7xl mx-auto px-6">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {vipPackages[activePackage] && (
                <GlassCard className="relative overflow-hidden" tilt={false}>
                  <div className="flex flex-col lg:flex-row">
                    {/* Package visual */}
                    <div className="relative lg:w-1/2 h-64 lg:h-80 overflow-hidden bg-[#0D1F3C] flex items-center justify-center">
                      <div
                        className="absolute inset-0"
                        style={{
                          background: `radial-gradient(ellipse at 50% 50%, rgba(201,169,110,0.15) 0%, transparent 70%)`,
                        }}
                      />
                      <div className="text-8xl opacity-30">
                        {vipPackages[activePackage].id.includes('royal') ? '👑' :
                         vipPackages[activePackage].id.includes('coast') ? '⛵' : '✈️'}
                      </div>
                      <div className="absolute top-4 left-4">
                        <span className="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest bg-gradient-to-r from-[#E8CC9A] to-[#C9A96E] text-[#030812]">
                          {vipPackages[activePackage].badge}
                        </span>
                      </div>
                      <div className="absolute bottom-4 right-4 text-right">
                        <p className="text-xs text-[rgba(245,240,232,0.5)] mb-1">{language === 'ar' ? 'يبدأ من' : 'From'}</p>
                        <p className="text-2xl font-black text-gradient-gold">
                          {formatPrice(vipPackages[activePackage].price, currency)}
                        </p>
                    ...
                    <div className="lg:w-1/2 p-8 flex flex-col justify-between">
                      <div>
                        <h3
                          className="text-2xl font-black text-[rgba(245,240,232,0.95)] mb-3"
                        ...
                        <div className="flex flex-wrap gap-2 mb-6">
                          {vipPackages[activePackage].includes.map((item) => (
                            <span
                              key={item}
                              className="px-3 py-1.5 rounded-lg text-xs glass border border-[rgba(201,169,110,0.2)] text-[rgba(245,240,232,0.7)] flex items-center gap-1.5"
                            >
                      ...
                      <button
                        ...
                        className="w-full py-4 rounded-xl text-sm font-bold tracking-widest uppercase text-[#030812] bg-gradient-to-r from-[#E8CC9A] to-[#C9A96E] hover:from-[#C9A96E] hover:to-[#A07840] transition-all duration-300 glow-gold"
                      >
                        {tr('trips.bookNow')}
                      </button>
                    </div>
        ...
        {/* Filter */}
        <div className="flex gap-3 mb-10 overflow-x-auto pb-2 justify-center flex-wrap">
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-xs font-medium tracking-wider whitespace-nowrap transition-all duration-300 ${
                activeFilter === cat.key
                  ...
                  : 'glass border border-[rgba(201,169,110,0.2)] text-[rgba(245,240,232,0.6)] hover:text-[#C9A96E] hover:border-[rgba(201,169,110,0.4)]'
              }`}
            >
              {cat.label[language]}
            </button>
            ...
            {filtered.map((trip, i) => (
              ...
              >
                <GlassCard className="group h-full flex flex-col">
                  {/* Card image area */}
                  <div className="relative h-52 overflow-hidden bg-[#0D1F3C] flex items-center justify-center">
                    <div
                      className="absolute inset-0 transition-transform duration-700 group-hover:scale-110"
                      style={{
                        background: `radial-gradient(ellipse at 50% 50%, ${categoryColors[trip.category]}22 0%, transparent 70%)`,
                      }}
                    />
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 py-3 rounded-xl text-xs font-bold tracking-wider uppercase bg-gradient-to-r from-[#E8CC9A] to-[#C9A96E] text-[#030812] hover:opacity-90 transition-opacity"
                      >
                        {tr('trips.bookNow')}
                      </button>
                      <button className="px-3 py-3 rounded-xl glass border border-[rgba(201,169,110,0.2)] text-[rgba(245,240,232,0.5)] hover:text-[#C9A96E] transition-colors">
